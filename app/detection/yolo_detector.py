...
class YoloDetector(Detector):
    def __init__(
        ...
        self._precision = self._resolve_precision(config.precision)
    ...
    def _resolve_precision(
        ...
        if requested == "auto":
            return "fp16" if self._device != "cpu" else "fp32"

        if requested == "fp16":
            if self._device == "cpu":
                raise ValueError("FP16 was requested, but the current device is CPU.")
